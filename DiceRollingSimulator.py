# This Python file uses the following encoding: utf-8
# http://knightlab.northwestern.edu/2014/06/05/five-mini-programming-projects-for-the-python-beginner/
# The Goal: Like the title suggests, this project involves writing a program that simulates rolling dice.
# When the program runs, it will randomly choose a number between 1 and 6.
# (Or whatever other integer you prefer — the number of sides on the die is up to you.)
# The program will print what that number is. It should then ask you if you’d like to roll again.
# For this project, you’ll need to set the min and max number that your dice can produce.
# For the average die, that means a minimum of 1 and a maximum of 6.
# You’ll also want a function that randomly grabs a number within that range and prints it.
#
# Concepts to keep in mind:
#
#     Random
#     Integer
#     Print
#     While Loops

import random

value = random.randrange( 1, 7, 1 )
print( value, '\n' )
print( 'Do you want to roll again?\n' )
rollAgain = 'no'
rollAgainAccepted = input( 'If you want roll, enter \'yes\'\n' )
if rollAgainAccepted == 'yes':
    rollAgain = 'yes'
else:
    rollAgain = 'no'
    print( 'It\'s a shame, you should\'ve tried again.' )
    quit( )

while rollAgain == 'yes':
    newValue = random.randrange( 1, 7, 1 )
    print( 'Your new value is ', newValue )
    print( 'Do you want to roll again? yes/no' )
    rollAgainAgain = input( '' )
    if rollAgainAgain == 'yes':
        continue
    else:
        print( 'Thank you for using Dice Express.' )
    break
else:
    print( 'Please come back again later' )
    quit( )
